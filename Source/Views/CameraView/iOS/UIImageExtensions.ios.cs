...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        /// <summary>
        /// ada hardcode CGPoint-nya ( Horizontal End + Vertical Center)
        /// </summary>
        /// <param name="sourceImage"></param>
        /// <param name="image"></param>
        /// <returns></returns>
        public static UIImage OverlayWith(this UIImage sourceImage, UIImage image)
        {
            var size = sourceImage.Size;
            var newSize = new CGSize(size.Width, size.Height);
            UIGraphics.BeginImageContextWithOptions(newSize, false, (nfloat)0.0);

            sourceImage.Draw(new CGRect(CGPoint.Empty, size));
            nfloat rightMargin = 4;
            image.Draw(new CGRect(
                                    new CGPoint(x: size.Width - image.Size.Width - rightMargin,
                                                y: size.Height / 2),
                                                size: image.Size));
        ...
        public static UIImage RotateImage(this UIImage sourceImage, float degree)
        {
            float Radians = degree * (float)Math.PI / 180;

            UIView view = new UIView(frame: new CGRect(0, 0, sourceImage.Size.Width, sourceImage.Size.Height));
            ...
            context.DrawImage(new CGRect(-sourceImage.Size.Width / 2, -sourceImage.Size.Height / 2, sourceImage.Size.Width, sourceImage.Size.Height), sourceImage.CGImage);
