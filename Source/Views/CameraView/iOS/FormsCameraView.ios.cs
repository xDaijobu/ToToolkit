...
{
	/// <summary>
    /// Source Code: https://github.com/xamarin/XamarinCommunityToolkit/blob/main/src/CommunityToolkit/Xamarin.CommunityToolkit/Views/CameraView/iOS/FormsCameraView.ios.cs
    /// </summary>
	public sealed class FormsCameraView : UIView, IAVCaptureFileOutputRecordingDelegate
	{
		...
		public FormsCameraView(MediaOptions mediaOptions)
		{
			...
			if (mediaOptions?.Placemark is not null)
			{
				var placemarkLabel = new UILabel();
				placemarkLabel.Text = mediaOptions.Placemark.ToString();
				placemarkLabel.TextColor = UIColor.White;
				placemarkLabel.Font = UIFont.SystemFontOfSize(7,5);
		...
		public void SwitchFlash(FlashMode newFlashMode)
		{
			if (isAvailable && device != null && newFlashMode != flashMode)
				UpdateFlash(newFlashMode);
		}

		void UpdateFlash(FlashMode? newFlashMode = null)
		{
			try
			{
		...
		bool CheckFlashModeSupported(FlashMode flashMode)
		{
			_ = device ?? throw new NullReferenceException();

			return flashMode switch
			{
				FlashMode.Off => device.IsFlashModeSupported(AVCaptureFlashMode.Off),
				FlashMode.On => device.IsFlashModeSupported(AVCaptureFlashMode.On),
				FlashMode.Auto => device.IsFlashModeSupported(AVCaptureFlashMode.Auto),
